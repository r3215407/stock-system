...
export default function ScreeningWorkspace() {
  ...
  return <main className={styles.page}><div className={styles.shell}>
    <header className={styles.hero}><div className={styles.heroMain}><p className={styles.eyebrow}>全市场策略签发中心</p><h1 className={styles.heroTitle}>筛选今日市场，<br/><em>签发 TOP 10</em></h1><p className={styles.heroCopy}>固定版本依次执行证券池、基础过滤、逐股评分与稳定排名。所有成功完成评分的股票统一参与排名，策略门槛只标记状态，不再阻止 TOP 10 返回。</p></div><div className={styles.heroSide}><strong>完整日线扫描</strong><p>结果写入 Redis 并保留 3 天；相同策略再次扫描时直接读取缓存。</p></div></header>
...
function CandidateList({candidates,selected,setSelected,detail}:{candidates:ScreeningCandidate[];selected:string[];setSelected:(value:string[])=>void;detail:(item:ScreeningCandidate)=>void}){
  ...
  return <div className={styles.table}><div className={styles.tableHead}><span>排名</span><span>股票</span><span>结论</span><span>总分</span><span>转强分</span><span>止损风险</span><span>20日均额</span><span>操作</span></div>{candidates.map((item)=><div className={styles.candidate} key={item.symbol}><span className={styles.rank}>#{item.rank??"—"}</span><div><button className={styles.stockButton} onClick={()=>detail(item)}>{item.name}</button><p className={styles.code}>{item.code} · {item.market}</p></div><span className={styles.pass}>{item.conclusion}</span><span className={styles.score}>{item.score}<small>/100</small></span><span>{item.strengthScore}/35</span><span>{(item.stopDistanceRate*100).toFixed(2)}%<br/><b>{item.riskLabel}</b></span><span>{money(item.averageAmount20)}</span><div><Link className={styles.detailLink} href={`/evaluate?symbol=${item.code}&signalDate=${item.signalDate}`}>详情</Link> <input aria-label={`选择${item.name}`} checked={selected.includes(item.symbol)} onChange={(e)=>setSelected(e.target.checked?[...selected,item.symbol]:selected.filter((v)=>v!==item.symbol))} type="checkbox"/></div></div>)}</div>;
}
