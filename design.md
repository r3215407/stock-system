...
- **未来**：用冰蓝、深海蓝、半透明层次和精确的数据排版建立科技感；
- **极简**：信息密度高但层级清楚，不靠装饰制造复杂感。
...
| `glacier-25` | `#F8FBFD` | 页面主背景 |
| `glacier-50` | `#F1F7FA` | 次级背景、表头、禁用表面 |
...
| `radius-md` | 10px | 输入框、按钮、表格容器 |
| `radius-lg` | 14px | 默认卡片 |
| `radius-xl` | 20px | 重点结果卡、弹窗 |
| `radius-pill` | 999px | 状态徽标，不用于普通按钮 |
...
shadow-md: 0 8px 24px rgba(16, 44, 58, 0.08)
shadow-lg: 0 20px 50px rgba(8, 28, 38, 0.12)
```

- 普通卡片优先使用边框，仅在悬浮层或粘性区域使用阴影；
- 同一页面不超过两种阴影层级；
- 不使用彩色外发光作为常驻效果。
...
审阅确认后，本文件升级为 `version: 1.0`，作为网站后续页面和组件的强制设计基准。
