...
Actions 页面的 `Daily stock screening` 也可手动运行。它先创建带当日幂等键的扫描任务，再逐个驱动该任务的 PostgreSQL 分片，直到生成双榜或达到安全上限。正常分片之间等待 10 秒。worker 请求返回 HTTP 错误，或当前分片中有任一股票详情获取失败时，Action 会立即退出并把原任务标记为暂停；已成功结果和失败记录都保留在原 PostgreSQL 任务中。页面会显示“重试未完成数据”，点击后优先重试失败股票，再继续原任务的后续分片。
